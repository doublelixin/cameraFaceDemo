...
import com.xin.camera.facedemo.log
import com.xin.camera.facedemo.toast
import com.xin.camera.facedemo.util.BitmapUtils
import com.xin.camera.facedemo.view.AutoFitTextureView
import java.util.*
...
class Camera2HelperFace(val mActivity: Activity, private val mTextureView: AutoFitTextureView) {
    ...
    private fun initFaceDetect() {
        ...
        log(
            "成像区域  ${activeArraySizeRect.width()}  ${activeArraySizeRect.height()} 比例: ${
                activeArraySizeRect.width().toFloat() / activeArraySizeRect.height()
            }"
        )
        ...
        if (faces != null) {
            for (face in faces) {
                ...
                val resultFaceRect = if (mCameraFacing == CaptureRequest.LENS_FACING_FRONT)
                    ...
                    RectF(
                        rawFaceRect.left - 100f,
                        rawFaceRect.top - 700f,// - mPreviewSize.width,
                        rawFaceRect.right + 50f,
                        rawFaceRect.bottom - 1350f// - mPreviewSize.width
                    )
